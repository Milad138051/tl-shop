					...
					@foreach($sms as $key => $item)
                        <tr>
                            ...
                            <td>{{jalaliDate($item->published_at,'H-i:s Y-m-d')}}</td>
							 ...
							 <td>
                                    <label>
                                        <input id="{{ $item->id }}" onchange="changeStatus({{ $item->id }})" data-url="{{ route('admin.notify.sms.status', $item->id) }}" type="checkbox" @if ($item->status === 1)
                                        checked
                                                @endif>
                                    </label>
                                </td>
						   ...
						   <td class="width-16-rem text-left">
                                <a href="{{ route('admin.notify.sms.edit',$item->id) }}" class="btn btn-info btn-sm"><i class="fa fa-edit"></i> ویرایش</a>
                                  
								  <form class="d-inline" action="{{ route('admin.notify.sms.destroy', $item->id) }}" method="post">
                                        @csrf
                                        {{ method_field('delete') }}
