                ...
                <table class="table table-striped table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <label>
                                    <input id="{{ $ticketCategory->id }}" onchange="changeStatus({{ $ticketCategory->id }})" data-url="{{ route('admin.ticket.category.status', $ticketCategory->id) }}" type="checkbox" @if ($ticketCategory->status === 1)
                                    checked
                                    @endif>
                                </label>
                            </td>
                ...
                success : function(response){
                    if(response.status){
                        if(response.checked){
                            ...
                            sweetalertStatusSuccess('دسته بندی با موفقیت فعال شد')
                        }
                        else{
                            element.prop('checked', false);
                            sweetalertStatusSuccess('دسته بندی با موفقیت غیر فعال شد')
                        }
                    }
                    else{
