                    ...
                    <section class="mt-3 mt-md-auto search-wrapper">
                        <section class="search-box">
                            <section class="search-textbox">
								...
								@csrf
                                 <input id="search" name="search" type="text" value="{{request()->search}}" class="" placeholder="جستجو ..." autocomplete="off">
								</form>
                            </section>
                        </section>
                    </section>
                    ...
                    <section class="mt-3 mt-md-auto text-end">
                        ...
                        <section class="header-cart d-inline ps-3 border-start position-relative">
                            <a class="btn btn-link position-relative text-dark header-cart-link" href="{{route('customer.Sales-process.cart')}}">
                                <i class="fa fa-shopping-cart"></i> <span style="top: 80%;" class="position-absolute start-0 translate-middle badge rounded-pill bg-danger">@if ($cartItems->count() > 0)
                                    {{$cartItems->count()}}
                                    @else
                                    0
                                @endif</span>
                            ...
                            <section class="header-cart-dropdown">
                                <section class="border-bottom d-flex justify-content-between p-2">
                                    <span class="text-muted">(@if ($cartItems->count() > 0)
                                        {{$cartItems->count()}}
                                        @else
                                        0
                                    @endif) کالا</span>
                                ...
                                <section class="header-cart-dropdown-body">
                                    ...
                                    $cartItemFinalPrice += $cartItem->cartItemFinalPrice();
                                    @endphp
                                    ...
                                    <section class="header-cart-dropdown-body-item d-flex justify-content-start align-items-center">
                                        ...
                                        <section class="w-100 text-truncate"><a class="text-decoration-none text-dark" href="{{route('customer.market.product',$cartItem->id)}}">{{$cartItem->product->name}} ({{ $cartItem->number }})</a></section>
                                        <section class="flex-shrink-1"><a class="text-muted text-decoration-none p-1" onclick="return confirm('آیا آیتم مورد نظر حذف شود')" href="{{route('customer.Sales-process.remove-from-cart',$cartItem)}}"><i class="fa fa-trash-alt"></i></a></section>
                                    </section>
                                ...
                                <section class="header-cart-dropdown-footer border-top d-flex justify-content-between align-items-center p-2">
                                    <section>
                                        ...
                                        <section> {{ priceFormat($cartItemFinalPrice) }} تومان</section>
                                    </section>
                                    <section class=""><a class="btn btn-danger btn-sm d-block" href="#">ثبت سفارش</a></section>
                                </section>
                            </section>
                        </section>
                     @endauth
