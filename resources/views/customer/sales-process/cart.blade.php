                ...
                <section class="row mt-4">
                            ...
                            <section class="cart-item d-md-flex py-3">
                                ...
                                <section class="align-self-start w-100">
                                    ...
                                    <p>
									 ...
									 <span>( {{priceFormat($cartItem->product->marketable_number)}} ) عدد موجود در انبار </span>
									</p>
                                    <section>
									
                                        <section class="cart-product-number d-inline-block">
                                            <button class="cart-number cart-number-down" type="button">-</button>
                                            <input class="number" name="number[{{$cartItem->id}}]" type="number" min="1" max="5" step="1" data-product-price={{ $cartItem->cartItemProductPrice() }} data-product-discount={{ $cartItem->cartItemProductDiscount() }} value="{{ $cartItem->number }}" data-max={{$cartItem->product->marketable_number}}  readonly="readonly">
                                            <button class="cart-number cart-number-up" type="button">+</button>
                                        </section>
										
										

                                        <a class="text-decoration-none ms-4 cart-delete"  onclick="return confirm('آیا آیتم مورد نظر حذف شود');"
                                        class="badge badge-warning" href="{{route('customer.Sales-process.remove-from-cart',$cartItem)}}"><i class="fa fa-trash-alt"></i> حذف از سبد</a>
                                    </section>
                                </section>
                    ...
                    <section class="col-md-3">
                        <section class="content-wrapper bg-white p-3 rounded-2 cart-total-price">
                            <section class="d-flex justify-content-between align-items-center">
                                <p class="text-muted">قیمت کالاها (@if ($cartItems->count() > 0)
                                    {{$cartItems->count()}}
                                    @else
                                    0
                                @endif)
								</p>
                                <p class="text-muted" id="total_product_price"><span>{{ priceFormat($totalProductPrice) }}</span> </p>
                            </section>
                ...
                <section class="content-wrapper bg-white p-3 rounded-2">
                    ...
                    <section class="lazyload-wrapper" >
                        <section class="lazyload light-owl-nav owl-carousel owl-theme">
							...
							@else
							<h3 class="text-danger">محصولی یافت نشد</h1>
